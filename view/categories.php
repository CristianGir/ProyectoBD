...
if (isset($_POST["eliminar"])) {
    $id = $_POST["id"];
    $query = "DELETE FROM categorias WHERE ide_cat = '$id'";
    mysqli_query($conexion, $query);
}

if (isset($_POST["searchTerm"])) {
    $searchTerm = $_POST["searchTerm"];
    $categories = "SELECT * FROM categorias WHERE ide_cat LIKE '%$searchTerm%'";
    $result = mysqli_query($conexion, $categories);
} else {
...
<body>
    ...
    <div class="container">
        <div class="table_header">
            <h2>Categorias</h2>
            <button><a href="../controller/addCategory.php">Agregar Categoria</a></button>
            <button><a href="../index.html">Volver</a></button>
            <div class="input_search">
            <form method="POST" action="" onsubmit="showSpinner()">
                    <div class="search-wrapper">
                        <input type="search" name="searchTerm" placeholder="Buscar por nombre categoria" />
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>".$row["ide_cat"]."</td>";
                        echo "<td>".$row["des_cat"]."</td>";
                        echo "<td>";
                        echo "<form method='POST' action=''>";
                        echo "<input type='hidden' name='id' value='" . $row["ide_cat"] . "'>";

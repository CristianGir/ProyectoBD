<?php
    ...
    if(isset($_POST["eliminar"])) {
        $id = $_POST["id"];
        $query = "DELETE FROM vendedor WHERE ide_ven = '$id'";
        mysqli_query($conexion, $query);
    } 

    if (isset($_POST["searchTerm"])) {
        $searchTerm = $_POST["searchTerm"];
        $vendedores = "SELECT * FROM vendedor WHERE ide_ven LIKE '%$searchTerm%'";
        $result = mysqli_query($conexion, $vendedores);
    } else {
        $vendedores = "SELECT * FROM vendedor";
        $result = mysqli_query($conexion, $vendedores);
    }    
?>
...
<body>
    ...
    <div class="container">
        <div class="table_header">
            <h2>Vendedores</h2>
            <button><a href="../controller/addSeller.php">Agregar vendedor</a></button>
            <button><a href="../index.html">Volver</a></button>
            <div class="input_search">
                <form method="POST" action="" onsubmit="showSpinner()">
                    <div class="search-wrapper">
                        <input type="search" name="searchTerm" placeholder="Buscar por ID del vendedor" />
    ...
    <tbody>
        ...
        if(mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td>".$row["ide_ven"]."</td>";
                echo "<td>".$row["nom_ven"]."</td>";
                echo "<td>".$row["tel_ven"]."</td>";
                echo "<td>".$row["email_ven"]."</td>";
                echo "<td>";
                echo "<form method='POST' action=''>";
                echo "<input type='hidden' name='id' value='" . $row["ide_ven"] . "'>";
